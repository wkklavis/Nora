...
class BinaryFocalLoss(nn.Module):
    def __init__(self, alpha=0.25, gamma=2, size_average=True):
        """
        Focal Loss for binary classification tasks (input is probabilities)
        :param alpha: balance factor for positive/negative samples
        :param gamma: focusing parameter to down-weight easy samples
    ...
    def forward(self, preds, labels):
        """
        :param preds: probabilities after sigmoid, shape [B, 1, H, W]
        :param labels: ground truth binary labels, shape [B, 1, H, W]
        :return: focal loss
        """
        # Remove the channel dimension for consistency
        ...
        # Compute focal loss
        labels = labels.float()
        pt = torch.where(labels == 1, preds, 1 - preds)  # Probabilities for the true class
