...
class BUSIDataset(Dataset):
    def __init__(self, root, list_path, num_class, joint_augment=None, augment=None, target_augment=None, lb_change=True):
        self.joint_augment = joint_augment
        self.augment = augment
        self.lb_change = lb_change
        self.num_class = num_class
        # self.name_list = img_list
        self.target_augment = target_augment
        self.root = root
        self.img_dir = os.path.join(root, 'images')
        self.lb_dir = os.path.join(root, 'masks')

        item_list_filepath = list_path
        self.nlist = [id.strip().split('.')[0] for id in open(item_list_filepath)]

    def __len__(self):
        return len(self.nlist)

    def __getitem__(self, index):
        img_name = self.nlist[index]
        # load image
        ...
        lbl_file = os.path.join(self.lb_dir, img_name + '_mask.png')
...
class DatasetBDataset(Dataset):
    def __init__(self, root, num_class, joint_augment=None, augment=None, target_augment=None, lb_change=True):
        ...
        self.lb_dir = os.path.join(root, 'label')

        self.nlist = [name.strip().split('.')[0] for name in os.listdir(os.path.join(root, 'image'))]
...
class STUDataset(Dataset):
    def __init__(self, root, num_class, joint_augment=None, augment=None, target_augment=None, lb_change=True):
        ...
        self.nlist = [name.strip().split('.')[0] for name in os.listdir(os.path.join(root, 'img'))]
...
if __name__ == '__main__':
    means = [0, 0, 0]
    stdevs = [0, 0, 0]
    path = '/home/whdong/data/Dataset_BUSI_with_GT/BUSI_WO_normal'
    train_json = '/home/whdong/data/Dataset_BUSI_with_GT/BUSI_WO_normal/train_list.json'
    ...
    target_transform = transforms.Compose([
        transforms.ToTensor()])
    train_set = BUSIDataset(root='cfg.DATASET.DATA_DIR', img_list='cfg.DATASET.TRAIN_LIST',
...
# resize256:
# normMean = [84.43374294 84.43310749 84.41932206]
# normStd = [51.77725727 51.77767919 51.76840112]
# transforms.Normalize(normMean = [84.43374294 84.43310749 84.41932206], normStd = [51.77725727 51.77767919 51.76840112])
# =============
# 84.42872416382653
# 51.77444585806658

# resize400:
# normMean = [84.43387835 84.43324547 84.41943367]
# normStd = [52.06118214 52.06177021 52.05159634]
# transforms.Normalize(normMean = [84.43387835 84.43324547 84.41943367], normStd = [52.06118214 52.06177021 52.05159634])
# =============
# 84.42885249748997
# 52.05818289907683